...
CREATE TABLE IF NOT EXISTS `dbMecanica`.`clientes` (
  ...
  INDEX `fk_clientes_carros1_idx` (`veiculo_idVeiculo` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `dbMecanica`.`servicos` (
  ...
  INDEX `fk_servicos_peças1_idx` (`peças_idPeças` ASC) VISIBLE,
  INDEX `fk_servicos_funcionarios1_idx` (`funcionarios_idfuncionario` ASC) VISIBLE,
  CONSTRAINT `fk_servicos_carros1`
    FOREIGN KEY (`idVeiculo`)
    REFERENCES `dbMecanica`.`veiculo` (`idVeiculo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_servicos_peças1`
    FOREIGN KEY (`peças_idPeças`)
    REFERENCES `dbMecanica`.`pecas` (`idPeca`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `dbMecanica`.`pecas` (
  ...
  PRIMARY KEY (`idPeca`),
  INDEX `fk_peças_servicos1_idx` (`servicos_idServico` ASC) VISIBLE,
  CONSTRAINT `fk_peças_servicos1`
    FOREIGN KEY (`servicos_idServico`)
    REFERENCES `dbMecanica`.`servicos` (`idServico`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
